...
const useStyles = makeStyles({
	content: (props: ProductContentProps) => ({
		flex: props.inCart ? 2 : '',
		display: 'flex',
		textAlign: 'left',
		flexFlow: 'column',
...
export default function ProductContent(props: ProductContentProps) {
	...
	const cartItem = cartItems && cartItems.find((item: { productId: number }) => item.productId === product.id);
	const { setIsLoading } = useLoadingContext() || {};

	function updateCart(type: string) {
		if (setIsLoading) {
			setIsLoading(true);
		}
		switch (type) {
			case 'plus':
				updateCartItem({
					variables: { id: cartId, amount: amount ? amount + 1 : 0 },
				});
				break;
			case 'minus':
				updateCartItem({
	...
	const showAddOrDeleteIcon = inCart ? (
		<IconButton className={classes.iconButtom} aria-label="delete" onClick={() => updateCart('delete')}>
			<DeleteForeverIcon color="disabled" />
		</IconButton>
	) : (
		<Fab color="primary" size="small" onClick={() => updateCart('add')}>
